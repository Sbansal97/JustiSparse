...
def main():
    ...
    def get_attr_labels(dataset):
        labels = set(dataset.features['protected_attributes'])
        labels = list(labels)
        return labels
    ...
    if training_args.do_train:
        ...
        attr_label_list = get_attr_labels(train_dataset)
    ...
    num_attr_labels = len(label_list)
    attr_label2id = {l: i for i, l in enumerate(attr_label_list)}
    ...
    def preprocess_function(examples):
        ...
        tokenized_examples['attr'] = [
            label2id.get(label, label)
            for label in examples['protected_attributes']
        ]
