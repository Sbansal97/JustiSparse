...
class DataTrainingArguments:
    ...
    """
    protected_attribute_column: str = field(
        default='protected_attribute_column',
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, SftArguments, TrainingArguments, RegArguments))
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script and it's the path to a json file,
        # let's parse it to get our arguments.
        model_args, data_args, sft_args, training_args, reg_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
        model_args, data_args, sft_args, training_args, reg_args = parser.parse_args_into_dataclasses()
    ...
    if training_args.do_train:
        ...
        protected_aattribute_list = list(set(raw_datasets["train"][data_args.protected_attribute_column]))
        protected_attribute2id = {l: i for i, l in enumerate(protected_aattribute_list)}
    else:
        column_names = raw_datasets["validation"].column_names
        protected_aattribute_list = list(set(raw_datasets["validation"][data_args.protected_attribute_column]))
        protected_attribute2id = {l: i for i, l in enumerate(protected_aattribute_list)}
