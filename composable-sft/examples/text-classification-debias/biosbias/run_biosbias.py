...
def main():
    ...
    if debias_args.debias_configuration != "none":
        task_ft = SFT()
        tensors = torch.load(debias_args.diffs_path, map_location=training_args.device)
        ...
        if not diffs and not abs:
            logger.warn(f'Empty SFT {debias_args.diffs_path}')
    ...
    elif sft_args.task_ft is not None:
    ...
    trainer = trainer_cls(
        ...
        evaluate_with_diffs=debias_args.debias_configuration == 'after'
