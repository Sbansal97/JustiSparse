...
mkdir -p models/$DATASET/$PEFT/$DEBIAS_METHOD-before
...
python run_extrinsic.py \
  ...
  --output_dir models/$DATASET/$PEFT/$DEBIAS_METHOD-before \
  --do_train \
  --do_eval \
  --do_predict \
  ...
  --save_total_limit 2 \
  --log_level debug \
  --cls_weights $CLASS_WEIGHTS > models/$DATASET/$PEFT/$DEBIAS_METHOD-before/training.log
